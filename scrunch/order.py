...
class NestedProjectsOrder(object):
    def __init__(self, project_res):
        self.project_res = project_res

    def create_project(self, name):
        # This should be a method of the Project class
        from scrunch.datasets import Project
        proj_res = self.project_res.create(shoji_entity_wrapper({
            'name': name
        }))
        return Project(proj_res)

    def get(self, path):
        self.project_res.refresh()  # Always up to date
        node = self
    ...
    def get_child(self, name):
        from scrunch.datasets import Project
        by_name = self.project_res.by('name')
