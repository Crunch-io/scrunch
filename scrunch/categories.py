...
class Category(ReadOnly):
    ...
    def delete(self):
        if self.resource.body.get("derivation"):
            raise TypeError(
                "Cannot delete categories on derived variables. Re-derive with the appropriate expression"
            )

        categories = [
            cat for cat in self.resource.body["categories"] if cat["id"] != self.id
        ]
    ...
    def edit(self, **kwargs):
        ...
        if extra_attrs:
            raise AttributeError(
                "Cannot edit the following attributes: %s" % ", ".join(extra_attrs)
            )

        categories = [
            self.as_dict(**kwargs) if cat["id"] == self.id else cat
...
class CategoryList(OrderedDict):
    ...
    def _from(cls, variable_resource):
        ...
        categories = [
            (cat["id"], Category(variable_resource, cat))
