...
DATASET_DOC = {
    "body": {
        ...
        "table": {
            ...
            "metadata": {
                "identity": {"alias": "identity", "name": "ID", "type": "numeric"},
                "ip_address": {
                    "alias": "ip_address",
                ...
                "hobbies": {
                    ...
                    "name": "Hobbies",
                    "subvariables": [
                        {"alias": "hobbies_1", "name": "Sports"},
                        {"alias": "hobbies_2", "name": "Video Games"},
                        {"alias": "hobbies_3", "name": "Reading"},
                        {"alias": "hobbies_4", "name": "Outdoor Activities"},
                    ],
                ...
                "music": {
                    ...
                    "name": "Music",
                    "subvariables": [
                        {"alias": "music_1", "name": "Pop"},
                        {"alias": "music_2", "name": "Rock"},
                        {"alias": "music_97", "name": "Other"},
                        {"alias": "music_98", "name": "Don't know"},
                        {"alias": "music_99", "name": "None of these"},
                    ],
                ...
                "religion": {
                    ...
                    "categories": [
                        ...
                        {"id": 5, "missing": False, "name": "None", "numeric_value": 5},
                        {
                            "id": 6,
                            "missing": False,
                            "name": "Other",
...
def invalid_credentials():
    return any(item is None for item in (CRUNCH_URL, CRUNCH_USER, CRUNCH_PASSWORD))
...
def main():
    ...
    try:
        ...
        df = pandaslib.dataframe(dataset.resource)
        valid_ids = [row[0] for row in ROWS if row[0] != "identity" and row[4] != 32766]
        ...
        df = pandaslib.dataframe(dataset.resource)
        valid_ids = [
            row[0] for row in ROWS if row[0] != "identity" and 32766 not in row[5:9]
        ]
        assert len(df) == len(valid_ids)
        for _, row in df.iterrows():
            assert row["identity"] in valid_ids
        ...
        df = pandaslib.dataframe(dataset.resource)
        valid_ids = [
            row[0] for row in ROWS if row[0] != "identity" and 32766 in row[5:9]
        ]
        assert len(df) == len(valid_ids)
        for _, row in df.iterrows():
            assert row["identity"] in valid_ids
        ...
        valid_ids = [
            row[0]
            for row in ROWS
            if row[0] != "identity" and 32766 not in row[5:9] and 32767 not in row[5:9]
        ]
        assert len(df) == len(valid_ids)
        for _, row in df.iterrows():
            assert row["identity"] in valid_ids
            assert {"?": 32766} not in row["hobbies"] and {"?": 32767} not in row[
                "hobbies"
            ]
        ...
        df = pandaslib.dataframe(dataset.resource)
        valid_ids = [
            row[0] for row in ROWS if row[0] != "identity" and 32766 not in row[9:14]
        ]
        assert len(df) == len(valid_ids)
        for _, row in df.iterrows():
            assert row["identity"] in valid_ids
        ...
        df = pandaslib.dataframe(dataset.resource)
        valid_ids = [
            row[0] for row in ROWS if row[0] != "identity" and 1 not in row[9:14]
        ]
        assert len(df) == len(valid_ids)
        for _, row in df.iterrows():
            assert row["identity"] in valid_ids
        ...
        valid_ids = [
            row[0]
            for row in ROWS
            if row[0] != "identity" and 1 not in row[9:14] and 2 not in row[9:14]
        ]
        assert len(df) == len(valid_ids)
        for _, row in df.iterrows():
            assert row["identity"] in valid_ids
        ...
        for _, row in df.iterrows():
            assert row["identity"] in valid_ids
            assert row["music"] != [1, 1, 1, 1, 1] and row["music"] != [2, 2, 2, 2, 2]
        ...
        for _, row in df.iterrows():
            assert row["identity"] in valid_ids
            assert row["music"] == [1, 1, 1, 1, 1] or row["music"] == [2, 2, 2, 2, 2]
        ...
        df = pandaslib.dataframe(dataset.resource)
        valid_ids = [
            row[0] for row in ROWS if row[0] != "identity" and row[4] in (32766, 32767)
        ]
        assert len(df) == len(valid_ids)
        for _, row in df.iterrows():
            assert row["identity"] in valid_ids
        ...
        for _, row in df.iterrows():
            assert row["identity"] in valid_ids
            assert {"?": 32766} in row["hobbies"] or {"?": 32767} in row["hobbies"]
        ...
        dataset.exclude(expr)
        df = pandaslib.dataframe(dataset.resource)
        valid_ids = [row[0] for row in ROWS if row[0] != "identity" and row[5] != 4]
        assert len(df) == len(valid_ids)
        for _, row in df.iterrows():
            assert row["identity"] in valid_ids
        ...
        bilingual_ids = set(
            row[0] for row in ROWS if row[0] != "identity" and row[4] in (2, 3)
        )
        assert (
            set(
                int(r["identity"])
        ...
        non_bilingual_ids = set(
            row[0] for row in ROWS if row[0] != "identity" and row[4] in (1, 4)
        )
        assert (
            set(
                int(r["identity"])
        ...
        assert (
            set(int(r["identity"]) for _, r in df[df["bilingual"].isnull()].iterrows())
            == bilingual_null_ids
        )
        ...
        cat_names = {1: "Interested", 2: "Not interested", 99: "Unknown"}
        ...
        cat_names = {1: "music_recoded_1", 2: "music_recoded_2", 3: "music_recoded_3"}
