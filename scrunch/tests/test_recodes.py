...
def _any(self, column):
    return {"function": "any", "args": [{"variable": self}, {"column": column}]}
...
def mr_in(mr_self, mr_alias, groups, parent_subvariables):
    ...
    """
    return _any(
        mr_self, [parent_subvariables[subvar_alias(mr_alias, sv)].id for sv in groups]
    )
...
CATEGORY_MAP = {1: [2, 3], 2: 1}
CATEGORY_NAMES = {1: "China", 2: "Other"}

RESPONSE_MAP = {1: [1, 2]}
RESPONSE_NAMES = {1: "online"}
            ...
            "args": [
                {"variable": var_url},
                {
                    "value": [
                        {
                            "alias": "alias_1",
                            "combined_ids": [subvar1_url, subvar2_url],
                            "name": "online",
                        }
                    ]
                },
...
class TestCombine(TestCase):
    def test_validate_range_expression(self):
        ...
        modified_map = responses_from_map(
            parent_var, test_map, test_cats, "test", "parent"
        )
    ...
    def test_validate_integer(self):
        ...
        subvar_mock = mock.MagicMock(entity_url=subvar1_url)
        # mock the call to entity, this will happen on Variable.resource
        variable_mock.entity.subvariables.by.return_value = {"parent_1": subvar_mock}
        parent_var = Variable(variable_mock, ds_res_mock)
        modified_map = responses_from_map(
            parent_var, test_map, test_cats, "test", "parent"
        )
    ...
    def test_combine_categories_unknown_alias(self):
        ...
        with pytest.raises(ValueError) as err:
            ds.combine_categorical(
                "unknown", CATEGORY_MAP, CATEGORY_NAMES, name="name", alias="alias"
            )

        assert (
            "Entity mocked_dataset has no (sub)variable with a name or alias unknown"
            in str(err.value)
        )
    ...
    def test_combine_categories_from_alias(self):
        ...
        with pytest.raises(ValueError) as err:
            ds.combine_categorical(
                "test", CATEGORY_MAP, CATEGORY_NAMES, name="name", alias="alias"
            )
    ...
    def test_combine_categories_from_entity(self):
        ...
        with pytest.raises(ValueError) as err:
            ds.combine_categorical(
                entity, CATEGORY_MAP, CATEGORY_NAMES, name="name", alias="alias"
            )
        ds.resource.variables.create.assert_called_with(RECODES_PAYLOAD)
        assert "Entity mocked_dataset has no (sub)variable" in str(err.value)
    ...
    def test_combine_responses_unknown_alias(self):
        ...
        with pytest.raises(ValueError) as err:
            ds.combine_multiple_response(
                "test", RESPONSE_MAP, RESPONSE_NAMES, name="name", alias="alias"
            )
    ...
    def test_combine_responses_by_entity(self):
        ...
        with pytest.raises(ValueError) as err:
            ds.combine_multiple_response(
                entity_mock, RESPONSE_MAP, RESPONSE_NAMES, name="name", alias="alias"
            )
        resource.variables.create.assert_called_with(COMBINE_RESPONSES_PAYLOAD)
        assert "Entity mocked_dataset has no (sub)variable" in str(err.value)
...
class TestRecode(TestCase):
    ...
    def test_recode_categoricals(self, get_dataset_mock):
        categories = [
            {"missing": False, "name": "Heterosexual", "numeric_value": 1, "id": 1},
            {"missing": False, "name": "Gay or lesbian", "numeric_value": 2, "id": 2},
            {"missing": False, "name": "Bisexual", "numeric_value": 3, "id": 3},
            {"missing": False, "name": "Other", "numeric_value": 4, "id": 4},
            {
                "missing": False,
            ...
            {"missing": True, "name": "skipped", "numeric_value": None, "id": 8},
            {"missing": True, "name": "not asked", "numeric_value": None, "id": 9},
        ]
    ...
    def test_recode_multiple_responses(self, get_dataset_mock):
        ...
        subvariables = {"Q1_1": subvar_mock, "Q1_2": subvar_mock, "Q1_3": subvar_mock}
