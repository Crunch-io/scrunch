...
from pycrunch.shoji import Entity, TaskError, TaskProgressTimeoutError
...
from scrunch.helpers import (NO_DATA_ID, NOT_SELECTED_ID, SELECTED_ID,
                             ReadOnly, _validate_category_rules, abs_url,
                             case_expr, download_file, else_case_not_selected,
                             generate_subvariable_codes, get_else_case,
                             shoji_catalog_wrapper, shoji_entity_wrapper,
                             subvar_alias, valid_categorical_date,
                             validate_categories)
...
class Project:
    ...
    def get(self, path):
        from scrunch.order import InvalidPathError, Path
...
class DatasetVariablesMixin(Mapping):
    ...
    def order(self):
        ...
        if "hier" not in self._catalog.orders:
            raise TypeError(
                "This dataset does not expose a hierarchical order. Use .folders"
            )
...
class BaseDataset(ReadOnly, DatasetVariablesMixin):
    ...
    def create_fill_values(self, variables, name, alias, description=""):
        ...
        fill_map = {
            str(cid): {"variable": vars_by_alias[v["variable"]]["id"]}
        ...
        fill_expr = {"function": "fill", "args": [expr, {"map": fill_map}]}
    ...
    def bind_categorical_array(
        ...
        array_map = {
            v["id"]: {"variable": self.get_url_by_alias(v["alias"])}
    ...
    def copy_variable(
        self, variable, name, alias, derived=None, subvariable_codes=None
    ):
        """
    ...
    def export(
        ...
        if "var_label_field" in options and options["var_label_field"] not in (
            "name",
            "description",
        ):
            raise ValueError(
    ...
    def add_filter(self, name, expr, public=False):
        ...
        payload = shoji_entity_wrapper(
            dict(name=name, expression=expression, is_public=public)
        )
    ...
    def backfill_from_csv(
        self, aliases, pk_alias, csv_fh, rows_filter=None, timeout=None
    ):
        """
...
class Variable(ReadOnly, DatasetSubvariablesMixin):
    ...
    def add_category(
        self, id, name, numeric_value, missing=False, date=None, before_id=False
    ):
        if self.resource.body["type"] not in CATEGORICAL_TYPES:
            raise TypeError(
                "Variable of type %s do not have categories" % self.resource.body.type
    ...
    def _subtotal_headings(self, operation, name, categories, anchor, negative=None):
        ...
        payload = {"view": {"transform": {"insertions": insertions}}}
        self.resource.patch(payload)
        self.dataset._reload_variables()
        return self.dataset[self.alias]
...
class BackfillFromCSV:
    ...
    def __init__(self, dataset, pk_alias, aliases, rows_expr, timeout=None):
        ...
        self.tmp_aliases = {a: "{}-{}".format(dataset.id, a) for a in aliases}
    ...
    def create_tmp_ds(self, csv_file):
        ...
        tmp_ds = self.root.datasets.create(
        ...
        try:
            ...
            if exc.status_code == 400:
                ...
                raise ValueError(
                    "Error importing CSV file - Columns should match specified types"
                )
            elif exc.status_code == 413:
                raise ValueError(
                    "Upload failed because the CSV file is too large. Limit is 150MB"
                )
            else:
        ...
        tmp_ds.variables.patch(
            shoji_catalog_wrapper(
                {
                    tmp_aliases[a].entity_url: {"alias": self.tmp_aliases[a]}
    ...
    def join_tmp_ds(self, tmp_ds):
        ...
        pycrunch.shoji.wait_progress(
            resp, self.dataset.resource.session, progress_tracker=self.progress_tracker
        )
