...
class TestExpressions(BaseIntegrationTestCase):
    def _create_mr_dataset(self, name, rows):
        _dataset_metadata = {
            "mr_variable": {
                ...
                "categories": MR_CATS,
                "subreferences": [
                    {"name": "Response 1", "alias": "response_1"},
                    {"name": "Response 2", "alias": "response_2"},
                    {"name": "Response 3", "alias": "response_3"},
                ],
            }
        }
        ds = self.site.datasets.create(
            {
                ...
                "body": {
                    "name": name,
                    "table": {"element": "crunch:table", "metadata": _dataset_metadata},
                },
            }
        ).refresh()
    ...
    def test_multiple_response_all_add_filter_value(self):
        ...
        try:
            ...
            data = ds.follow("table", "limit=20&filter={}".format(resp.resource.self))[
                "data"
            ]
            ds_variables = ds.variables.by("alias")
            mr_variable_id = ds_variables["mr_variable"].id
            assert data[mr_variable_id] == [[1, 1, 1]]
        finally:
            # cleanup
            ds.delete()
    ...
    def test_multiple_response_any_add_filter_single_subvar(self):
        ...
        try:
            ...
            ds_variables = ds.variables.by("alias")
            mr_variable_id = ds_variables["mr_variable"].id
            assert data[mr_variable_id] == [[1, 2, 2]]
        finally:
            # cleanup
            ds.delete()
    ...
    def test_multiple_response_any_add_filter_subvar(self):
        ...
        try:
            ...
            ds_variables = ds.variables.by("alias")
            mr_variable_id = ds_variables["mr_variable"].id
            assert data[mr_variable_id] == [[2, 1, 1]]
        finally:
            # cleanup
            ds.delete()
    ...
    def test_categorical_array_any_add_filter(self):
        ds = self.site.datasets.create(
            as_entity({"name": "test_any_categorical_add_filter"})
        ).refresh()
        ds.variables.create(
            as_entity(
                {
                    ...
                    "subvariables": [
                        {"alias": "response_1", "name": "Response1"},
                        {"alias": "response_2", "name": "Response2"},
                        {"alias": "response_3", "name": "Response3"},
                    ],
                    "values": [[1, 3, -1], [2, 1, 1], [2, 3, 2]],
                }
            )
        )
        scrunch_dataset = get_mutable_dataset(ds.body.id, self.site)
        ...
        try:
            ...
            assert data[cat_var_variable_id] == [[1, 3, {"?": -1}], [2, 1, 1]]
                ...
                "args": [
                    {
                        "function": "in",
                        "args": [
                            {"var": "categorical_var", "axes": ["response_1"]},
                            {"value": [1]},
                        ],
                    },
                    {
                        "function": "in",
                        "args": [
                            {"var": "categorical_var", "axes": ["response_2"]},
                            {"value": [1]},
                        ],
                    },
                    {
                        "function": "in",
                        "args": [
                            {"var": "categorical_var", "axes": ["response_3"]},
                            {"value": [1]},
                        ],
                    },
                ],
    ...
    def test_categorical_array_any_w_bracket_subvar(self):
        ds = self.site.datasets.create(
            as_entity({"name": "test_any_categorical_w_bracket_add_filter"})
        ).refresh()
        cat_var = ds.variables.create(
            as_entity(
                {
                    "name": "Categorical Var",
                    "alias": "categorical_var",
    ...
    def test_append_dataset_any_filter_multiple_response(self):
        ...
        ds_to_append, scrunch_dataset_to_append = self._create_mr_dataset(
            "test_mr_any_to_append_subvar", ds_to_append_rows
        )
        ...
        try:
            ...
            assert data[mr_variable_id] == [[1, 2, 1], [1, 2, 2], [1, 1, 1], [2, 1, 2]]
        finally:
            # cleanup
            ds.delete()
            ds_to_append.delete()
    ...
    def test_categorical_any_add_filter_value(self):
        ds = self.site.datasets.create(
            as_entity({"name": "test_any_categorical_filter"})
        ).refresh()
        categories = [
    ...
    def test_categorical_any_add_filter_multiple_values(self):
        ds = self.site.datasets.create(
            as_entity({"name": "test_any_categorical_filter_multiple_values"})
        ).refresh()
        categories = [
            {"id": 1, "name": "One", "missing": False, "numeric_value": None},
            {"id": 2, "name": "Two", "missing": False, "numeric_value": None},
