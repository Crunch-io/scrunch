...
def setup_folders(ds):
    ...
    ds.variables.create(
        {"element": "shoji:entity", "body": {"name": "testvar1", "type": "numeric"}}
    )
    ds.variables.create(
        {"element": "shoji:entity", "body": {"name": "testvar2", "type": "numeric"}}
    )
    ds.variables.create(
        {"element": "shoji:entity", "body": {"name": "testvar3", "type": "numeric"}}
    )
    ds.variables.create(
        {"element": "shoji:entity", "body": {"name": "testvar4", "type": "numeric"}}
    )
    ds.variables.create(
        {"element": "shoji:entity", "body": {"name": "testvar5", "type": "numeric"}}
    )
    ds.variables.create(
        {"element": "shoji:entity", "body": {"name": "testvar6", "type": "numeric"}}
    )
    ds.refresh()
    sf1 = Catalog(sess, body={"name": "Subfolder 1"})
    sf1 = ds.folders.public.create(sf1)
    sfa = Catalog(sess, body={"name": "Subfolder A"})
    sfa = sf1.create(sfa)
    sf2 = Catalog(sess, body={"name": "Subfolder 2"})
    sf2 = ds.folders.public.create(sf2)
    variables = ds.variables.by("alias")
    sf1.patch({"index": {variables["age"].entity_url: {}}})
    sfa.patch({"index": {variables["gender"].entity_url: {}}})
...
class TestFolders(BaseIntegrationTestCase):
    ...
    def test_create_folder_with_variables(self):
        ...
        self.assertEqual(["testvar1", "testvar2"], [c.alias for c in sf.children])
    ...
    def test_reorder_folder(self):
        ...
        self.assertEqual(
            [c.url for c in children], [c.url for c in [sf1, sf2, sf3, var]]
        )
        self.assertEqual(list(map(type, children)), [Folder, Folder, Folder, Variable])

        # Reorder placing sf1 at the end
        folder.reorder([sf2, var, sf3, sf1])
        children = folder.children
        self.assertEqual(
            [c.url for c in children], [c.url for c in [sf2, var, sf3, sf1]]
        )

        # Test reorder providing children names only
        folder.reorder(var.alias, sf1.name, sf2.name, sf3.name)
        self.assertEqual(
            [c.url for c in folder.children], [c.url for c in [var, sf1, sf2, sf3]]
        )
    ...
    def test_move_between_folders(self):
        ...
        self.assertEqual([c.url for c in target1.children], [var2.url, var1.url])

        nested = target1.create_folder("nested")
        self.assertEqual(
            [c.url for c in target1.children], [var2.url, var1.url, nested.url]
        )

        # Move var2 to t2
        target2.move_here(var2)
        # Now, t1 does not have var2, it is under t2
        self.assertEqual([c.url for c in target1.children], [var1.url, nested.url])
        self.assertEqual([c.url for c in target2.children], [var2.url])
        ...
        self.assertEqual(
            [c.url for c in target2.children], [var2.url, nested.url, var1.url]
        )
        ...
        self.assertEqual([c.url for c in target2.children], [var2.url, nested.url])
        self.assertEqual([c.url for c in nested.children], [var1.url])
        ...
        self.assertEqual([c.url for c in target2.children], [var2.url])
        self.assertEqual([c.url for c in target1.children], [nested.url])
    ...
    def test_move_here_position(self):
        ...
        self.assertEqual(
            [c.name for c in sf.children], ["testvar2", "testvar3", "testvar1"]
        )
        sf.move_here(self.ds["testvar4"], position=1)
        self.assertEqual(
            [c.name for c in sf.children],
            ["testvar2", "testvar4", "testvar3", "testvar1"],
        )
    ...
    def test_make_folder_in_position(self):
        ...
        self.assertEqual([c.name for c in public.children], ["p1", "A", "p2", "p3"])

        public.create_folder("B", after="p2")
        self.assertEqual(
            [c.name for c in public.children], ["p1", "A", "p2", "B", "p3"]
        )

        public.create_folder("C", position=3)
        self.assertEqual(
            [c.name for c in public.children], ["p1", "A", "p2", "C", "B", "p3"]
        )
    ...
    def test_dict_protocol(self):
        ...
        self.assertEqual(
            {c.url for c in dict(public).values()}, {c.url for c in public.children}
        )
    ...
    def test_dict_protocol_on_helper(self):
        ...
        # __iter__ method
        self.assertEqual([c.url for c in top_level], [c.url for c in public.children])

        # more dict methods
        self.assertEqual(set(dict(top_level).keys()), set(public.keys()))
        self.assertEqual(
            {c.url for c in dict(top_level).values()}, {c.url for c in public.children}
        )
        # The right key corresponds to the correct value
