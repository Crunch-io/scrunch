...
class BaseTestCase(BaseIntegrationTestCase):
    ...
    def _export_dataset(self, ds):
        output = tempfile.NamedTemporaryFile(mode="w+t", delete=True)
        ds.export(output.name)
...
class BaseTestDatasets(BaseTestCase):
    ...
    def _test_export_dataset(self):
        ...
        ds, ds_instance = self._create_dataset(name="test_dataset")
        exported = self._export_dataset(ds)
